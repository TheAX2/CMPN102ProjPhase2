...
void CardFourteen::ResetBought()
{
	isBought == 0;
}
void CardFourteen::Setloaded(bool b)
{
	loaded = b;
}
void CardFourteen::Setsaved(bool b)
{
	saved = b;
}
...
void CardFourteen::Apply(Grid* pGrid, Player* pPlayer)
{
	...
	if (!isBought)
	{
		...
		if (pPlayer->GetWallet() > CardPrice) {
			...
			string temp = "You have reached CardFourteen, card's Price=" + to_string(CardPrice);
...
void CardFourteen::Save(ofstream& OutFile, int type, int& noofobj)
{
	if (type != 3)
	{
		if (noofobj == 0) {
