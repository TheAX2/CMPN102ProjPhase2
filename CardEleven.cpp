...
void CardEleven::Apply(Grid* pGrid, Player* pPlayer)
{
	...
	if (!isBought)
	{
		...
		if (pPlayer->GetWallet() > CardPrice) {
			...
			string temp = "You have reached CardEleven, card's Price=" + to_string(CardPrice);
...
void CardEleven::Save(ofstream& OutFile, int type, int& noofobj)
{
	if (type != 3)
	{
		if (noofobj == 0) {
