...
void CardThirteen::Apply(Grid* pGrid, Player* pPlayer)
{
	...
	if (!isBought)
	{
		...
		if (pPlayer->GetWallet() > CardPrice) {
			...
			string temp = "You have reached CardThirteen, card's Price=" + to_string(CardPrice);
...
void CardThirteen::Save(ofstream& OutFile, int type, int& noofobj)
{
	if (type != 3)
	{
		if (noofobj == 0) {
