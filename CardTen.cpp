...
void CardTen::ResetBought()
{
	isBought == 0;
}

void CardTen::Setloaded(bool b)
{
	loaded = b;
}
void CardTen::Setsaved(bool b)
{
	saved = b;
}
...
void CardTen::Apply(Grid* pGrid, Player* pPlayer)
{
	...
	if (!isBought)
	{
		...
		if (pPlayer->GetWallet() > CardPrice) {
			...
			string temp = "You have reached CardTen, card's Price=" + to_string(CardPrice);
...
void CardTen::Save(ofstream& OutFile, int type, int& noofobj)
{
	if (type != 3)
	{
		if (noofobj == 0) {
